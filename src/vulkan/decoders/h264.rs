...
pub trait H264Decoder {
    ...
    unsafe fn create_h264_session_parameters(device: &Device, video_loader: &video_queue::Device, session: vk::VideoSessionKHR) -> vk::VideoSessionParametersKHR;
}
impl H264Decoder for Aura {
    ...
    fn decode_frame(&mut self, bitstream_data: &[u8], is_first_frame: bool) {
            ...
            unsafe {
                let swapchain_sync_idx = (self.current_frame_index % self.swapchain_images.len()) as usize;
                let (image_index, _is_suboptimal) = self.swapchain_loader
                            .acquire_next_image(
                                self.swapchain,
                                u64::MAX,
                ...
                let signal_semaphores = [self.render_complete_semaphores[image_index as usize]];
                let begin_info = vk::CommandBufferBeginInfo::default()
                    .flags(vk::CommandBufferUsageFlags::ONE_TIME_SUBMIT);
                
                self.device.begin_command_buffer(self.video_command_buffer, &begin_info).unwrap();
                ...
                self.device.cmd_pipeline_barrier2(self.video_command_buffer, &dependency_info);
                
                let mut std_pic_info: vk::native::StdVideoDecodeH264PictureInfo = MaybeUninit::zeroed().assume_init();
                ...
                let mut std_setup_info: vk::native::StdVideoDecodeH264ReferenceInfo = MaybeUninit::zeroed().assume_init();
                std_setup_info.FrameNum = std_pic_info.frame_num;
                
                let mut h264_setup_slot_info_decode = vk::VideoDecodeH264DpbSlotInfoKHR::default()
                    .std_reference_info(&std_setup_info);
                    
                let mut h264_setup_slot_info_begin = vk::VideoDecodeH264DpbSlotInfoKHR::default()
                    .std_reference_info(&std_setup_info);
                ...
                let mut coding_reference_slots: Vec<vk::VideoReferenceSlotInfoKHR> = vec![setup_slot_begin];
                
                if !is_first_frame {
                    let max_refs = std::cmp::min(self.current_frame_index as u32, (self.dpb_pool_size as u32) - 1);
                    for i in 0..max_refs {
                        let ref_offset = (i + 1) as usize;
                        let ref_idx = (self.current_frame_index.wrapping_sub(ref_offset)) % self.dpb_pool_size;
                        
                        if ref_idx < self.current_frame_index {
                            let (_, _, view) = self.dpb_pool[ref_idx];
                            
                            let mut std_ref: vk::native::StdVideoDecodeH264ReferenceInfo = MaybeUninit::zeroed().assume_init();
                            std_ref.FrameNum = ((self.current_frame_index - ref_offset) % 16) as u16;
                            refs_std.push(std_ref);
                            
                            refs_resource.push(vk::VideoPictureResourceInfoKHR::default()
                ...
                self.video_loader.cmd_begin_video_coding(self.video_command_buffer, &begin_coding_info);
                
                if is_first_frame {
                    let control_info = vk::VideoCodingControlInfoKHR::default().flags(vk::VideoCodingControlFlagsKHR::RESET);
                    self.video_loader.cmd_control_video_coding(self.video_command_buffer, &control_info);
                }
                ...
                self.decode_loader.cmd_decode_video(self.video_command_buffer, &decode_info);
                
                // 7. End Coding & Submit Execution
                self.video_loader.cmd_end_video_coding(self.video_command_buffer, &vk::VideoEndCodingInfoKHR::default());
                self.device.end_command_buffer(self.video_command_buffer).expect("Erro buffer");
                
                let submit_info = vk::SubmitInfo::default().command_buffers(std::slice::from_ref(&self.video_command_buffer)).signal_semaphores(&signal_semaphores);
                self.device.queue_submit(self.video_queue, &[submit_info], vk::Fence::null()).unwrap();
    ...
    unsafe fn create_h264_session_parameters(_device: &Device, video_loader: &video_queue::Device, session: vk::VideoSessionKHR) -> vk::VideoSessionParametersKHR {
        ...
        std_sps.chroma_format_idc = vk::native::StdVideoH264ChromaFormatIdc_STD_VIDEO_H264_CHROMA_FORMAT_IDC_420;
        std_sps.pic_width_in_mbs_minus1 = (1920 / 16) - 1;
        std_sps.pic_height_in_map_units_minus1 = (1080 / 16) - 1;
        std_sps.max_num_ref_frames = 16;
    
        let std_pps: vk::native::StdVideoH264PictureParameterSet = unsafe { MaybeUninit::zeroed().assume_init() };
        ...
        let mut h264_create = vk::VideoDecodeH264SessionParametersCreateInfoKHR::default()
            .max_std_sps_count(1).max_std_pps_count(1).parameters_add_info(&add_info); 
    
        let params_info = vk::VideoSessionParametersCreateInfoKHR::default()
            .video_session(session).push(&mut h264_create); 
        unsafe {
            video_loader.create_video_session_parameters(&params_info, None).unwrap()
        }
    }
}