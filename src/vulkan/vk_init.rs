...
use ash::{vk, Entry, khr::video_queue,  khr::video_decode_queue::Device as VideoDecodeLoader, Instance};
...
impl DecodeExtensions {
    pub const H264: &'static CStr = unsafe { CStr::from_bytes_with_nul_unchecked(b"VK_KHR_video_decode_h264\0") };
    pub const H265: &'static CStr = unsafe { CStr::from_bytes_with_nul_unchecked(b"VK_KHR_video_decode_h265\0") };
    pub const AV1:  &'static CStr = unsafe { CStr::from_bytes_with_nul_unchecked(b"VK_KHR_video_decode_av1\0") };
}
...
impl Aura {
    pub fn new(window: &winit::window::Window) -> Self {
        ...
        let mut instance_extensions = ash_window::enumerate_required_extensions(window.display_handle().unwrap().as_raw())
        ...
        let instance = unsafe { entry.create_instance(&instance_create_info, None).expect("Failed to create a new vulkan instance.") };
        let surface_loader = ash::khr::surface::Instance::load(&entry, &instance);
        let surface_factory = ash_window::SurfaceFactory::new(&entry, &instance, window.display_handle().unwrap().as_raw()).unwrap();
        
        let surface = unsafe {
            ash_window::SurfaceFactory::create_surface(&surface_factory, window.window_handle().unwrap().as_raw(), None).unwrap()
        };
        ...
        let _debug_messenger = unsafe { _debug_utils_loader.create_debug_utils_messenger(&debug_info, None).expect("Failed to create a debug vulkan messenger.") };
        
        let (physical_device, (graphics_queue_family_index, decode_queue_family_index), supported_decoders) = Self::select_physical_device(&instance, &surface_loader, surface);
        ...
        let mut video_maintenance = vk::PhysicalDeviceVideoMaintenance1FeaturesKHR::default().video_maintenance1(true);
        let mut sampler_ycbcr_conversion = vk::PhysicalDeviceSamplerYcbcrConversionFeaturesKHR::default().sampler_ycbcr_conversion(true);
        
        let queue_priorities = [1.0f32];
        let queue_info = [vk::DeviceQueueCreateInfo::default().queue_family_index(decode_queue_family_index).queue_priorities(&queue_priorities), vk::DeviceQueueCreateInfo::default().queue_family_index(graphics_queue_family_index).queue_priorities(&queue_priorities)];
        ...
        let device = unsafe { instance.create_device(physical_device, &device_create_info, None).expect("Failed to create a logical device.") };
        ...
        let _ycbcr_conversion_info = vk::SamplerYcbcrConversionInfo::default()
        ...
        let DecodingSession { session, _session_memories, video_loader, decode_loader, session_parameters} = Self::setup_decoder(&instance, physical_device, &device, &video_instance_ext, &mut video_profile, decode_queue_family_index);

        
        let (bitstream_buffer, bitstream_memory, _) = Aura::create_bitstream_buffer(&instance, &video_instance_ext, physical_device, &device, &video_profile);
        let (swapchain_loader, swapchain, swapchain_images, swapchain_image_views, swapchain_format, swapchain_extent) = unsafe {
        ...
        let video_command_pool = unsafe {
            device.create_command_pool(&video_pool_info, None)
                .expect("Failed to create a Video Command Pool.")
        };
        let graphics_cmd_alloc = vk::CommandBufferAllocateInfo::default().command_pool(graphics_command_pool).command_buffer_count(1);
        let graphics_command_buffer = unsafe {device.allocate_command_buffers(&graphics_cmd_alloc).unwrap()[0] };
           
        let video_alloc_info = vk::CommandBufferAllocateInfo::default().command_pool(video_command_pool).level(vk::CommandBufferLevel::PRIMARY).command_buffer_count(1);
        let video_command_buffer = unsafe { device.allocate_command_buffers(&video_alloc_info).unwrap()[0] };
        let (dpb_pool, dst_pool) = Self::create_dpb_dst_pool(&instance, physical_device, &device, &mut video_profile, ycbcr_conversion, dpb_pool_size);
        let semaphore_create_info = vk::SemaphoreCreateInfo::default();
        let mut present_complete_semaphores = Vec::new();
        let mut render_complete_semaphores = Vec::new();
        for _ in 0..swapchain_images.len() {
            unsafe {
                present_complete_semaphores.push(device.create_semaphore(&semaphore_create_info, None).unwrap());
                render_complete_semaphores.push(device.create_semaphore(&semaphore_create_info, None).unwrap());
            }
        }
    ...
    fn create_dpb_dst_pool(instance: &Instance, physical_device: vk::PhysicalDevice, device: &ash::Device, video_profile: &mut vk::VideoProfileInfoKHR, ycbcr_conversion: vk::SamplerYcbcrConversion, dpb_pool_size: usize) -> (Vec<(vk::Image, vk::DeviceMemory, vk::ImageView)>, Vec<(vk::Image, vk::DeviceMemory, vk::ImageView)>){
        let _output_pool: Vec<(vk::Image, vk::DeviceMemory, vk::ImageView)> = Vec::with_capacity(dpb_pool_size);
        ...
        for i in 0..dpb_pool_size {
            ...
            let mut local_ycbcr_info = vk::SamplerYcbcrConversionInfo::default()
                .conversion(ycbcr_conversion);
            ...
            let dpb_mem_handle = if i == 0 { dpb_memory } else { vk::DeviceMemory::null() };
            let dst_mem_handle = if i == 0 { dst_memory } else { vk::DeviceMemory::null() };
    ...
    fn setup_decoder(instance: &Instance, physical_device: vk::PhysicalDevice, device: &ash::Device, video_instance_ext: &ash::khr::video_queue::Instance, video_profile: &mut vk::VideoProfileInfoKHR, queue_family_index: u32) -> DecodingSession {
        ...
        let session_parameters = unsafe { 
            Aura::create_h264_session_parameters(device, &video_loader, session) 
        };
    ...
    unsafe fn create_swapchain(
        ...
        ) -> (ash::khr::swapchain::Device, vk::SwapchainKHR, Vec<vk::Image>, Vec<vk::ImageView>, vk::Format, vk::Extent2D) {
            ...
            (swapchain_loader, swapchain, swapchain_images, swapchain_image_views, format.format, extent)
        }
        ...
        for pdevice in pdevices {
            ...
            for (index, prop) in props.iter().enumerate() {
                ...
                if prop.queue_flags.contains(vk::QueueFlags::VIDEO_DECODE_KHR) {
                    ...
                    let device_extensions_name = unsafe { instance.enumerate_device_extension_properties(pdevice).unwrap() };
                    for ext_name in device_extensions_name {
                        unsafe {
                            let ext_cstr = CStr::from_ptr(ext_name.extension_name.as_ptr());
                            if ext_cstr == DecodeExtensions::H264 { supported_codecs.h264 = true }
                            else if ext_cstr == DecodeExtensions::H265 { supported_codecs.h265 = true }
                            else if ext_cstr == DecodeExtensions::AV1 { supported_codecs.av1 = true };
                        }
                    }
                }
            ...
            if let (Some(g_idx), Some(v_idx)) = (graphics_index, decode_index) {
                log::info!("GPU successfully detected! Graphic queue: {}, Video Queue: {}", g_idx, v_idx);
                return (pdevice, (g_idx, v_idx), supported_codecs);
            }
        }
    
        log::error!("Your GPU needs to support both video decode queue and graphics queue with surface support.");
        std::process::abort();
    }

    pub fn find_memory_type(instance: &Instance, pdevice: vk::PhysicalDevice, type_filter: u32, props: vk::MemoryPropertyFlags) -> u32 {
        let mem_props = unsafe { instance.get_physical_device_memory_properties(pdevice) };
        for i in 0..mem_props.memory_type_count {
            if (type_filter & (1 << i)) != 0 && (mem_props.memory_types[i as usize].property_flags & props) == props { return i; }
        }
        0
    }
    ...
    fn drop(&mut self) {
        unsafe {
            ...
            if self.session_parameters != vk::VideoSessionParametersKHR::null() {
                self.video_loader.destroy_video_session_parameters(self.session_parameters, None);
            ...
            self.device.destroy_command_pool(self.video_command_pool, None);
            ...
            self.device.destroy_sampler_ycbcr_conversion(self.ycbcr_conversion, None);
            
            self.device.destroy_device(None);
            log::info!("VkDevice successfully destroyed.");

            self.surface_loader.destroy_surface(self.surface, None);
            self._debug_utils_loader.destroy_debug_utils_messenger(self._debug_messenger, None);
