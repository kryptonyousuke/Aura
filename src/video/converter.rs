...
pub fn avcc_to_annexb(data: &[u8], extradata: &[u8]) -> Vec<u8> {
    ...
    if extradata.len() > 6 {
        ...
        if let Some(&sps_byte) = extradata.get(i) {
            ...
            for _ in 0..num_sps {
                if i + 2 > extradata.len() { log::error!("The file needs at least 2 bytes to read the size."); std::process::exit(1); }
                let size = ((extradata[i] as usize) << 8) | extradata[i + 1] as usize;
                i += 2;
    
                if i + size > extradata.len() { log::error!("The extracted size doesn't match the known vec size."); std::process::exit(1); }
        ...
        if let Some(&pps_byte) = extradata.get(i) {
            ...
            for _ in 0..num_pps {
                if i + 2 > extradata.len() { log::error!("There's no space to read this PPS size."); std::process::exit(1); }
                let size = ((extradata[i] as usize) << 8) | extradata[i + 1] as usize;
                i += 2;
    
                if i + size > extradata.len() { log::error!("The PPS data doesn't exist in the vector."); std::process::exit(1); }
